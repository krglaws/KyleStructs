...
static int ks_treemapnode_new_tests() {
  ...
  if (tmn->key != key) {
    printf(
        "TEST 1: ks_treemapnode key contains unexpected ks_datacont value.\n");
    retval = -1;
  }
  if (tmn->value != val) {
    printf(
...
static int ks_treemapnode_remove_tests() {
  ...
  if (ks_treemapnode_remove(&root, key2) != -1) {
    printf(
        "TEST 2: treemap_remove() should return -1 when removing not-present "
        "node key.\n");
    retval = -1;
  }
  ...
  if (root != NULL) {
    printf(
        "TEST 4: Expected root node to be NULL after last treemap_remove().\n");
    retval = -1;
  }
...
static int ks_treemapnode_count_tests() {
  ...
  if (count != 4) {
    printf(
        "TEST 1: Unexpected ks_treemapnode_count() return value: %d. Expected: "
        "4.\n",
        count);
    retval = -1;
  }
  ...
  if (count != 0) {
    printf(
        "TEST 2: Unexpected ks_treemapnode_count() return value: %d. Expected: "
        "4.\n",
        count);
    retval = -1;
  }
...
static int ks_treemapnode_balance_tests() {
  ...
  if (height != 5) {
    printf(
        "TEST 1: Unexpected tree height after ks_treemapnode_balance(): %d. "
        "Expected: 5.\n",
        height);
    retval = -1;
  }
