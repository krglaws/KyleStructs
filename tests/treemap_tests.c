...
static int ks_treemap_copy_tests() {
    ...
    if (tm_copy->root->key->c != 'B') {
        printf(
            "TEST 1: Unexpected ks_datacont value in ks_treemap_copy() "
            "result\n");
        retval = -1;
    }
...
static int ks_treemap_add_tests() {
    ...
    if (tm->root->key->i != 2) {
        printf(
            "TEST 1: Unexpected key ks_datacont at root node: %d. Expected: "
            "2.\n",
            tm->root->key->i);
        retval = -1;
    }
    if (tm->root->value->c != 'B') {
        printf(
            "TEST 1: Unexpected value ks_datacont at root node: %d. Expected "
            "B.\n",
            tm->root->value->c);
        retval = -1;
    }
    ...
    if (ks_treemap_add(NULL, NULL, NULL) != -1) {
        printf(
            "TEST 4: ks_treemap_add() should return -1 when params are "
            "NULL.\n");
        retval = -1;
    }
...
static int ks_treemap_get_key_tests() {
    ...
    if (get_dc != NULL) {
        printf(
            "TEST 4: Expected OOB call to ks_treemap_get_key() to return "
            "NULL.\n");
        retval = -1;
    }
...
static int ks_treemap_height_tests() {
    ...
    if (height != 3) {
        printf(
            "TEST 1: Unexpected ks_treemap_height() return value: %d. "
            "Expected: "
            "3.\n",
            height);
        retval = -1;
    ...
    if (height != 0) {
        printf(
            "TEST 2: Unexpected ks_treemap_height() return value: %d. "
            "Expected: "
            "0.\n",
            height);
        retval = -1;
...
static int ks_treemap_balance_tests() {
    ...
    if (height != 26) {
        printf(
            "TEST 1: ks_treemap_height() should have returned 26. Can't "
            "continue, "
            "quitting test...\n");
        ks_treemap_delete(tm);
        return -1;
