...
static int ks_hashmap_copy_tests() {
    ...
    ks_hashmap_add(hm, ks_datacont_new("B", KS_CHAR, 1),
    ...
    if (hm_copy == NULL) {
        printf(
            "TEST 1: Unexpected NULL return from ks_hashmap_copy() result\n");
        return -1;
    }
    ...
    if (hm_copy->buckets[0] != NULL) {
        if (hm_copy->buckets[0]->root->key->c != 'B') {
            printf(
                "TEST 1: Unexpected ks_datacont value in ks_hashmap_copy() "
                "result\n");
            retval = -1;
        }
...
static int ks_hashmap_add_tests() {
    ...
    if (ks_hashmap_add(hm, key, val) != 0) {
        printf(
            "TEST 1: ks_hashmap_add() should return 0 when adding a new "
            "pair.\n");
        retval = -1;
    }
    ...
    if (ks_hashmap_add(hm, key_copy, val2) != 1) {
        printf(
            "TEST 2: ks_hashmap_add() should return 1 when replacing an "
            "existing "
            "pair.\n");
        retval = -1;
    }
...
static int ks_hashmap_remove_tests() {
    ...
    if (ks_hashmap_remove(hm, key) != 0) {
        printf(
            "TEST 1: ks_hashmap_remove() should return 0 on removal of "
            "existing "
            "pair.\n");
        retval = -1;
    }
    if (ks_hashmap_remove(hm, key_copy) != -1) {
        printf(
            "TEST 2: ks_hashmap_remove() should return -1 when removing "
            "not-present pair.\n");
        retval = -1;
    }
...
static int ks_hashmap_get_key_tests() {
    ...
    ks_treemap_add(tm0, ks_datacont_new("B", KS_CHAR, 1),
                   ks_datacont_new("B", KS_CHAR, 1));
    ks_treemap_add(tm0, ks_datacont_new("A", KS_CHAR, 1),
                   ks_datacont_new("A", KS_CHAR, 1));
    ks_treemap_add(tm0, ks_datacont_new("C", KS_CHAR, 1),
                   ks_datacont_new("C", KS_CHAR, 1));

    ks_treemap* tm1 = ks_treemap_new();
    ks_treemap_add(tm1, ks_datacont_new("E", KS_CHAR, 1),
                   ks_datacont_new("E", KS_CHAR, 1));
    ks_treemap_add(tm1, ks_datacont_new("D", KS_CHAR, 1),
                   ks_datacont_new("D", KS_CHAR, 1));
    ks_treemap_add(tm1, ks_datacont_new("F", KS_CHAR, 1),
                   ks_datacont_new("F", KS_CHAR, 1));
    ...
    if (dc->c != 'F') {
        printf(
            "TEST 1: Unexpected ks_datacont value after ks_hashmap_get_key(): "
            "%c. "
            "Expected: 'F'\n",
            dc->c);
        retval = -1;
...
static int ks_hashmap_count_tests() {
    ...
    int one = 1;
    ks_hashmap_add(hm, ks_datacont_new(&one, KS_INT, 1),
                   ks_datacont_new("A", KS_CHAR, 1));

    int count = ks_hashmap_count(hm);
    if (count != 1) {
        printf(
            "TEST 1: Unexpected ks_hashmap_count() return value: %d. Expected "
            "1.\n",
            count);
        retval = -1;
    }
