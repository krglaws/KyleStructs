...
static int ks_hashmap_copy_tests() {
  ...
  if (hm_copy->buckets[0] != NULL) {
    if (hm_copy->buckets[0]->root->key->c != 'B') {
      printf(
          "TEST 1: Unexpected ks_datacont value in ks_hashmap_copy() result\n");
      retval = -1;
    }
...
static int ks_hashmap_add_tests() {
  ...
  if (ks_hashmap_add(hm, key, val) != 0) {
    printf(
        "TEST 1: ks_hashmap_add() should return 0 when adding a new pair.\n");
    retval = -1;
  }
  ...
  if (ks_hashmap_add(hm, key_copy, val2) != 1) {
    printf(
        "TEST 2: ks_hashmap_add() should return 1 when replacing an existing "
        "pair.\n");
    retval = -1;
  }
...
static int ks_hashmap_remove_tests() {
  ...
  if (ks_hashmap_remove(hm, key) != 0) {
    printf(
        "TEST 1: ks_hashmap_remove() should return 0 on removal of existing "
        "pair.\n");
    retval = -1;
  }
...
static int ks_hashmap_get_key_tests() {
  ...
  if (dc->c != 'F') {
    printf(
        "TEST 1: Unexpected ks_datacont value after ks_hashmap_get_key(): %c. "
        "Expected: 'F'\n",
        dc->c);
    retval = -1;
  }
...
static int ks_hashmap_count_tests() {
  ...
  if (count != 1) {
    printf(
        "TEST 1: Unexpected ks_hashmap_count() return value: %d. Expected 1.\n",
        count);
    retval = -1;
  }
