...
static int ks_treesetnode_copy_tests() {
  ...
  if (tsn_copy->dc->c != 'B') {
    printf(
        "TEST 1: Unexpected ks_datacont value after ks_treesetnode_copy()\n");
    retval = -1;
  }
...
static int ks_treesetnode_add_tests() {
  ...
  if (ks_treesetnode_add(tsn, left) != 0) {
    printf(
        "TEST 1: Unexpected error on adding (should be) left node to tsn.\n");
    retval = -1;
  }
  ...
  if (tsn->left != NULL) {
    if (tsn->left->dc->c != 'A') {
      printf(
          "TEST 1: Unexpected container value on left node: %c. Expected: "
          "%c.\n",
          tsn->left->dc->c, 'A');
      retval = -1;
    }
  ...
  if (ks_treesetnode_add(tsn, right) != 0) {
    printf(
        "TEST 2: Unexpected error on adding (should be) left node to tsn.\n");
    retval = -1;
  }
  if (tsn->right != NULL) {
    if (tsn->right->dc->c != 'C') {
      printf(
          "TEST 2: Unexpected container value on right node: %c. Expcted: "
          "%c.\n",
          tsn->right->dc->c, 'C');
      retval = -1;
    }
...
static int ks_treesetnode_remove_by_tests() {
  ...
  if (ks_treesetnode_contains(tsn, dc_D)) {
    printf(
        "TEST 2: ks_treesetnode should not contain 'D' ks_datacont value.\n");
    retval = -1;
  }
  ...
  if (ks_treesetnode_contains(tsn, dc_B)) {
    printf(
        "TEST 3: ks_treesetnode should not contain 'B' ks_datacont value.\n");
    retval = -1;
  }
  ...
  if (ks_treesetnode_contains(tsn, dc_A)) {
    printf(
        "TEST 4: ks_treesetnode should not contain 'A' ks_datacont value.\n");
    retval = -1;
  }
...
static int ks_treesetnode_remove_at_tests() {
  ...
  if (ks_treesetnode_remove_at(&tsn, 10) != -1) {
    printf(
        "TEST 1: ks_treesetnode_remove_by() should return 1 when 'index' is "
        "OOB.\n");
    retval = -1;
  }
  ...
  if (ks_treesetnode_contains(tsn, dc_A)) {
    printf(
        "TEST 2: ks_treesetnode should not contain 'A' ks_datacont value.\n");
    retval = -1;
  }
  ...
  if (ks_treesetnode_contains(tsn, dc_G)) {
    printf(
        "TEST 4: ks_treesetnode should not contain 'G' ks_datacont value.\n");
    retval = -1;
  }
  ks_datacont_delete(dc_Z);
  ks_datacont_delete(dc_D);
...
static int ks_treesetnode_index_tests() {
  ...
  if ((index = ks_treesetnode_index(tsn, dcA)) != 0) {
    printf(
        "TEST 1: Unexpected value returned from ks_treesetnode_index(): %d. "
        "Expected: 0.\n",
        index);
    retval = -1;
  }
  ...
  if ((index = ks_treesetnode_index(tsn, dcB)) != 1) {
    printf(
        "TEST 2: Unexpected value returned from ks_treesetnode_index(): %d. "
        "Expected: 1.\n",
        index);
    retval = -1;
  }
  ...
  if ((index = ks_treesetnode_index(tsn, dcC)) != 2) {
    printf(
        "TEST 3: Unexpected value returned from ks_treesetnode_index(): %d. "
        "Expected: 2.\n",
        index);
    retval = -1;
  }
...
static int ks_treesetnode_get_tests() {
  ...
  if (get_dc->c != 'C') {
    printf(
        "TEST 1: Unexpected ks_datacont value after ks_treesetnode_get(): %c. "
        "Expected: C.\n",
        get_dc->c);
    retval = -1;
  }
  ...
  if (get_dc->c != 'B') {
    printf(
        "TEST 2: Unexpected ks_datacont value after ks_treesetnode_get(): %c. "
        "Expected: B.\n",
        get_dc->c);
    retval = -1;
  }
  ...
  if (get_dc != NULL) {
    printf(
        "TEST 3: Unexpected ks_datacont value after OOB ks_treesetnode_get(): "
        "%c. Expected: NULL.\n",
        get_dc->c);
    retval = -1;
  }
...
static int ks_treesetnode_count_tests() {
  ...
  if (count != 3) {
    printf(
        "TEST 1: Unexpected value returned from ks_treesetnode_count(): %d. "
        "Expected: 3.\n",
        count);
    retval = -1;
  }
  ...
  if (count != 4) {
    printf(
        "TEST 2: Unexpected value returned from ks_treesetnode_count(): %d. "
        "Expected 4.\n",
        count);
    retval = -1;
  }
  ...
  if (count != 0) {
    printf(
        "TEST 3: Unexpected value returned from ks_treesetnode_count(): %d. "
        "Expected 0.\n",
        count);
    retval = -1;
  }
...
static int ks_treesetnode_height_tests() {
  ...
  if (height != 0) {
    printf(
        "TEST 2: Unexpected result from ks_treesetnode_height() on NULL param: "
        "%d. Expected: 0.\n",
        height);
    retval = -1;
  }
...
static int ks_treesetnode_balance_tests() {
  ...
  if (height != 26) {
    printf(
        "TEST 1: ks_treesetnode_height() should have returned 16. Returned: "
  ...
  if (height != 5) {
    printf(
        "TEST 1: Unexpected tree height after ks_treesetnode_balance(): %d. "
        "Expected: 5.\n",
        height);
    retval = -1;
  }
