...
static int ks_list_copy_tests() {
    ...
    /* TEST 2 */
    if (ls_copy->head->dc->c != 'A' ||
        ls_copy->head->next->next->dc->c != 'C') {
...
static int ks_list_add_tests() {
    ...
    if (ks_list_add(NULL, dc) != -1) {
        printf(
            "TEST 1: ks_list_add() with any NULL parameters should return "
            "-1\n");
        retval = -1;
    }
    ...
    if (ls->head->dc->c != 'A') {
        printf(
            "TEST 2: unexpected ks_datacont value at first ks_list: %c. "
            "Expected: "
            "A\n",
            ls->head->dc->c);
        retval = -1;
    ...
    if (ls->head->next->dc->c != 'B') {
        printf(
            "TEST 2: unexpected ks_datacont value at second ks_list: %c. "
            "Expected: "
            "B\n",
            ls->head->next->dc->c);
        retval = -1;
...
static int ks_list_remove_by_tests() {
    ...
    if (ks_list_remove_by(ls, dcC) != 1) {
        printf(
            "TEST 1: ks_list_remove_by() should return 1 on successful "
            "removal\n");
        retval = -1;
    }
    ...
    if (ks_list_remove_by(ls, dcC) != 0) {
        printf(
            "TEST 2: ks_list_remove_by() should return 0 when removing "
            "not-present "
            "ks_datacont\n");
        retval = -1;
    }
    ...
    if (ks_list_remove_by(ls, dcB) != 1) {
        printf(
            "TEST 3: ks_list_remove_by() should return 1 on successful "
            "removal\n");
        retval = -1;
    }
    ...
    if (ks_list_remove_by(ls, dcA) != 1) {
        printf(
            "TEST 4: ks_list_remove_by() should return 1 on successful "
            "removal\n");
        retval = -1;
    }
...
static int ks_list_remove_at_tests() {
    ...
    if (ks_list_remove_at(ls, -1) != 0) {
        printf(
            "TEST 1: ks_list_remove_at() should return 0 on successful "
            "removal\n");
        retval = -1;
    }
    ...
    if (ks_list_remove_at(ls, 0) != 0) {
        printf(
            "TEST 3: ks_list_remove_at() should return 0 on successful "
            "removal\n");
        retval = -1;
    }
    ...
    if (ks_list_remove_at(ls, -1) != 0) {
        printf(
            "TEST 4: ks_list_remove_at() should return 0 on successful "
            "removal\n");
        retval = -1;
    }
    ...
    if (ks_list_remove_at(ls, 0) != -1) {
        printf(
            "TEST 5: ks_list_remove_at() should return -1 when params are "
            "NULL\n");
        retval = -1;
    }
...
static int ks_list_remove_all_tests() {
    ...
    if (ks_list_remove_all(ls, dcC) != -1) {
        printf(
            "TEST 5: ks_list_remove_all() should return -1 when either param "
            "is "
            "NULL\n");
        retval = -1;
    }
...
static int ks_list_replace_by_tests() {
    ...
    if (ks_list_replace_by(ls, dcB, dcB) != -1) {
        printf(
            "TEST 2: ks_list_replace_by() should return -1 when trying to "
            "remove "
    ...
    if (ks_list_replace_by(NULL, NULL, NULL) != -1) {
        printf(
            "TEST 3: ks_list_replace_by() should return -1 when params are "
            "NULL\n");
        retval = -1;
    }
...
static int ks_list_replace_at_tests() {
    ...
    if (ks_list_replace_at(ls, dcB, 10) != -1) {
        printf(
            "TEST 2: ks_list_replace_at() should return -1 when index is "
            "OOB\n");
        retval = -1;
    }
    ...
    if (ks_list_replace_at(NULL, NULL, 0) != -1) {
        printf(
            "TEST 3: ks_list_replace_at() should return -1 when params are "
            "NULL\n");
        retval = -1;
    }
...
static int ks_list_insert_tests() {
    ...
    if (ks_list_insert(ls, ks_datacont_copy(dcB), 0) != 0) {
        printf(
            "TEST 1: ks_list_insert() should return 0 on successful "
            "insertion\n");
        retval = -1;
    }
    ...
    if (ks_list_insert(ls, ks_datacont_copy(dcB), -1) != 0) {
        printf(
            "TEST 2: ks_list_insert() should return 0 on successful "
            "insertion\n");
        retval = -1;
    }
    ...
    if (ks_list_insert(ls, NULL, 2) != -1) {
        printf(
            "TEST 3: ks_list_insert() should return -1 when any params are "
            "NULL\n");
        retval = -1;
    }
...
static int ks_list_push_tests() {
    ...
    if (ks_list_push(NULL, NULL) != -1) {
        printf(
            "TEST 3: ks_list_push() should return -1 when params are NULL\n");
        retval = -1;
    }
...
static int ks_list_peek_tests() {
    ...
    if (ks_list_remove_at(ls, 0) != 0) {
        printf(
            "TEST 1: ks_list_peek() should not remove from ks_list, only "
            "return "
            "reference.\n");
        retval = -1;
    }
...
static int ks_list_enqueue_tests() {
    ...
    if (ks_list_enqueue(NULL, NULL) != -1) {
        printf(
            "TEST 3: ks_list_enqueue() should return -1 when params are "
            "NULL\n");
        retval = -1;
    }
...
static int ks_list_dequeue_tests() {
    ...
    if (dc->c != 'A') {
        printf(
            "TEST 1: Unexpected ks_datacont* returned from ks_list_dequeue(): "
            "%c. "
            "Expected: A\n",
            dc->c);
        retval = -1;
    ...
    if (dc->c != 'B') {
        printf(
            "TEST 2: Unexpected ks_datacont* returned from ks_list_dequeue(): "
            "%c. "
            "Expected: B\n",
            dc->c);
        retval = -1;
    ...
    if (dc->c != 'C') {
        printf(
            "TEST 3: Unexpected ks_datacont* returned from ks_list_dequeue(): "
            "%c. "
            "Expected: C\n",
            dc->c);
        retval = -1;
    ...
    if (dc != NULL) {
        printf(
            "TEST 4: ks_list_dequeue() should return NULL on empty ks_list\n");
        retval = -1;
    }
