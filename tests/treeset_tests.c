...
static int ks_treeset_new_tests() {
    ...
    if (ts == NULL) {
        printf(
            "TEST 1: Unexpected NULL value returned from ks_treeset_new().\n");
        retval = -1;
    }
...
static int ks_treeset_copy_tests() {
    ...
    if (ts_copy->root->dc->c != 'A' || ts_copy->root->right->dc->c != 'B') {
        printf(
            "TEST 1: Unexpected ks_datacont values after ts_treeset_copy()\n");
        retval = -1;
    }
...
static int ks_treeset_add_tests() {
    ...
    if ((result = ks_treeset_add(ts, ks_datacont_copy(dcB))) != 0) {
        printf(
            "TEST 1: Unexpected return value from ks_treeset_add(): %d. "
            "Expected: "
            "0.\n",
            result);
        retval = -1;
    ...
    if ((result = ks_treeset_add(ts, dcB)) != 1) {
        printf(
            "TEST 2: Unexpected return value from ks_treeset_add(): %d. "
            "Expected "
            "1.\n",
            result);
        retval = -1;
    ...
    if ((result = ks_treeset_add(NULL, NULL)) != -1) {
        printf(
            "TEST 3: Unexpected return value from ks_treeset_add(): %d. "
            "Expected "
            "-1.\n",
            result);
        retval = -1;
...
static int ks_treeset_contains_tests() {
    ...
    if (ks_treeset_contains(ts, NULL) != 0) {
        printf(
            "TEST 3: ks_treeset_contains() should return 0 on NULL params.\n");
        retval = -1;
    }
...
static int ks_treeset_get_tests() {
    ...
    if (get_dc != NULL) {
        printf(
            "TEST 3: Unexpected ks_datacont value after OOB ks_treeset_get(): "
            "%c. "
            "Expected: NULL.\n",
            get_dc->c);
        retval = -1;
...
static int ks_treeset_height_tests() {
    ...
    if (height != 0) {
        printf(
            "TEST 2: Unexpected result from ks_treeset_height() on NULL param: "
            "%d. "
            "Expected: 0.\n",
            height);
        retval = -1;
...
static int ks_treeset_balance_tests() {
    ...
    if (height != 26) {
        printf(
            "TEST 1: ks_treeset_height() should have returned 26. Can't "
            "continue, "
            "quitting test...\n");
        ks_treeset_delete(ts);
        return -1;
