...
int ks_listnode_remove_by(ks_listnode** ln, const ks_datacont* dc) {
  ...
  if ((*ln)->next) return ks_listnode_remove_by(&((*ln)->next), dc);
  return 0;
}
...
int ks_listnode_remove_at(ks_listnode** ln, int index) {
  ...
  if ((*ln)->next) return ks_listnode_remove_at(&((*ln)->next), index - 1);
  return -1;
}
...
int ks_listnode_remove_all(ks_listnode** ln, const ks_datacont* dc) {
  ...
  if ((*ln)->next) return ks_listnode_remove_all(&(*ln)->next, dc);
  return 0;
}
...
static int __ks_listnode_index(const ks_listnode* ln, const ks_datacont* dc,
                               int curr_index) {
  if (ln == NULL || dc == NULL) return -1;

  if (ks_datacont_compare(dc, ln->dc) == KS_EQUAL) return curr_index;
...
const ks_datacont* ks_listnode_get(const ks_listnode* ln, int index) {
  if (ln == NULL || (index < 0 && (index = ks_listnode_length(ln) + index) < 0))
    return NULL;
...
unsigned int ks_listnode_count(const ks_listnode* ln, const ks_datacont* dc) {
  ...
  if (ks_datacont_compare(dc, ln->dc) == KS_EQUAL) {
    if (ln->next) return 1 + ks_listnode_count(ln->next, dc);
    return 1;
  }

  if (ln->next) return ks_listnode_count(ln->next, dc);
  return 0;
}
